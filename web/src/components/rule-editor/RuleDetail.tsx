...
export function RuleDetail(props: Props) {
  ...
  return (
    <div className="cc-re-detail">
      {rule ? <RuleBody {...props} rule={rule} /> : <NoSelection onAdd={props.onAdd} />}
      ...
      <div className="cc-re-savebar">
        ...
        <button className="cc-re-btn ghost" onClick={props.onRevert} disabled={!props.dirty}>
          REVERT
        </button>
        <button className="cc-re-btn primary" onClick={props.onSave} disabled={!props.dirty || !props.valid}>
          SAVE ALL
        </button>
      </div>
    </div>
...
function NoSelection({ onAdd }: { onAdd: () => void }) {
  return (
    <div className="cc-re-noselect">
      ...
      <button className="cc-re-noselect-add" onClick={onAdd}>
        + NEW RULE
      </button>
    </div>
  );
